...
The Docker build installs Python packages with `uv` on both architectures.
On `linux/arm64`, it falls back to `uv pip` for `pyepics` and `pvapy`, and
attempts `pcaspy` as best effort on that architecture.